/**
 * @type -> Permite limitar mis valores de la variable
 * @interface -> Estructura que permite definir un tipo de dato
 */
 type Currency = 'MXN' | 'USD' | 'PEN' | 'EUR';
...
class Expenses implements IExpenses{
  ...
  getTotal(): string {
    const total = this.expenses.getAll().reduce((acc:number, item:ExpenseItem) => {
